...
class YouTubeSearcher:
    ...
    def get_video_list_by_search(
        self, user_query: str, max_results: int = 2
    ) -> List[str]:
        ...
        while True:
            ...
            logging.info(
                f"The number of items added to the list: {len(video_urls)}", video_urls
            )
        ...
        for url in video_urls:
            ...
            try:
                ...
                if not blob.exists():
                    ...
                    logging.info(
                        f"Uploaded transcript for video {video_id} to {self.bucket_name}/{blob_path}"
                    )
    ...
    def upload_dicts_to_gcs(self, folder_name: str, data_list: List[Dict]):
        ...
        for data in data_list:
            ...
            try:
                ...
                if blob.exists():
                    logging.info(
                        f"File {blob} already exists in bucket {self.bucket_name}"
                    )
                else:
                    # Upload the JSON string to the blob if it does not exist
                    blob.upload_from_string(data_str)
                    logging.info(
                        f"Uploaded {video_id}.json to {folder_name}/ in bucket {self.bucket_name}"
                    )
