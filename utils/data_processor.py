...
class TranscriptProcessor:
    ...
    def read_transcript_to_df(self) -> pd.DataFrame:
        ...
        blobs = bucket.list_blobs(
            prefix=f"{self.folder_name}/{self.folder_name}_transcripts/"
        )
        rows = [
            {
                "video_id": blob.name.split("/")[-1],
                "transcripts": blob.download_as_bytes().decode("utf-8"),
            }
            for blob in blobs
        ]
    ...
    def clean_transcript_text(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        df["clean_transcript"] = df["transcripts"].apply(
            self._lambda_clean_transcript_text
        )
    ...
    def count_number_of_tokens(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        df["num_tokens"] = [
            self._num_tokens_from_string(transcript, "cl100k_base")
            for transcript in df["clean_transcript"]
        ]
        logging.info("Number of tokens per transcript counted.")
        return df

    def filter_rows_by_token_length(
        self, df: pd.DataFrame, threshold_tokens: int = 8192
    ) -> pd.DataFrame:
        """
    ...
    def upload_clean_transcript(self, df: pd.DataFrame) -> None:
        ...
        logging.info(
            "The upload of clean transcripts into their new folder is complete."
        )
    ...
    def upload_clean_transcript1(self, df: pd.DataFrame) -> None:
        ...
        for _, row in df.iterrows():
            ...
            ):  # Check if the transcript is not just whitespace
                blob_name = (
                    f"{folder_name}/{video_id}"  # Ensure to include the file extension
                )
                blob = bucket.blob(blob_name)
                if not blob.exists():
                    blob.upload_from_string(clean_transcript, "text/plain")
                    logging.info(
                        f"Uploaded clean transcript for video {video_id} to {blob_name}."
                    )
                else:
                    logging.info(f"Skipping {blob_name} as it already exists.")
            else:
